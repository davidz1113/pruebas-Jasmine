...
fdescribe('FormComponent', () => {
  ...
  describe('when addAsset is executed', () => {
    it('Should add new group', () => {
      const asset = <FormArray>component.secondFormGroup.get('assets');
      component.addAsset();
      component.addAsset();
      expect(Object.keys(asset.controls)).toEqual(['0', '1']);
    })
  })
  ...
  describe('When deleteAsset', () => {
    it('should remove the form control', () => {
      ...
      expect(Object.keys(asset.controls)).toEqual([]);
    });

    describe('when savePins is executed', () => {
      it('shoul nabigate to pins view', () => {
